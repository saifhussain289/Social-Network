...
if(isset($_POST['submit'])){
	...
	$location = "file/";
	$uploaded = move_uploaded_file($_FILES["image"]["tmp_name"], $location.$image);
  
	 $stream= mysql_real_escape_string($_POST['stream']) ;
	  $sem= mysql_real_escape_string($_POST['sem']) ;
	
$query = "INSERT INTO `time_table` (`stream`,`sem`,`image`) VALUES
('$stream','$sem','$image')";
	 $query_run = mysql_query($query);
			if($query_run)
			{
				$success_msg = "Time Table has been Submitted Successfully !";
		          ("<SCRIPT LANGUAGE='JavaScript'>
					window.alert('$success_msg')
					window.location.href='time-table.php';
					</SCRIPT>");
			}
					}
//mysql_query($qury);
